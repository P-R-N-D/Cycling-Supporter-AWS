...
<body>
<H1>ERROR</H1>
HTTP GET method is not permitted!
</body>
<%
...
for(int i=0; i<rs.size(); i++)
{
	...
	for(int j=0; j<rs.get(i).size(); j++)
	{
		...
		location+=(String)Double.toString((Double)rs.get(i).get(j));
	...
	<script type='text/javascript' charset='utf-8' defer='defer'>
   		...
   		*/
   		DrawLine.initMap = function() {
   			...
   			DrawLine.map = new Tmap.Map({
   				...
   				height : "350px",  // map의 height 설정
   				httpsMode: true		// map의 HTTPS 설정
   			});
   			//이동한도로찾기 API 사용할 지도 생성
   			DrawLine.map2 = new Tmap.Map({
   			...
   			// 이동한도로찾기 API 사용전 백터 레이어 생성
   			DrawLine.vectorLayer = new Tmap.Layer.Vector('TmapVectorLayer');
   			DrawLine.map.addLayers([DrawLine.vectorLayer]); // 지도에 백터 레이어 추가
   			
   			// 이동한도로찾기 API 사용할 백터 레이어 생성
   			DrawLine.vectorLayer2 = new Tmap.Layer.Vector('TmapVectorLayer');
   			DrawLine.map2.addLayers([DrawLine.vectorLayer2]); // 지도에 백터 레이어 추가
   			
   			//요청 좌표 표시 포인트
   			DrawLine.vector_layer = new Tmap.Layer.Vector('Tmap Vector Layer'); // 백터 레이어 생성
   			DrawLine.map.addLayers([DrawLine.vector_layer]); // 지도에 백터 레이어 추가
		...
		DrawLine.splitPoint = function() {
			...
			for( i=DrawLine.currentIndex; i < cntAllPoint; i+=2 ) {
				...
				var coord = new Tmap.LonLat(DrawLine.arrPoint[i], DrawLine.arrPoint[i+1]).transform("EPSG:4326", "EPSG:3857");
   				var circle = new Tmap.Geometry.Circle(coord.lon, coord.lat, 20); // 원 생성
   				// 지도상에 그려질 스타일을 설정합니다
   				var style_red = {
   					fillColor:"#000000",
   					fillOpacity:0.2,
   				...
   				DrawLine.vector_layer.addFeatures([circleFeature]); // 원 백터 를 백터 레이어에 추가
		...
		*/
		DrawLine.setMapBound = function() {
			...
			DrawLine.map.zoomToExtent(bounds); // 매칭전 좌표가 한눈에 들어올 수 있는 지도 중심과 줌레벨 설정
   			DrawLine.map2.zoomToExtent(bounds); // 매칭후 좌표가 한눈에 들어올 수 있는 지도 중심과 줌레벨 설정
		}
